...
def calcular_ex_taylor_otimizado(x, n, k=0, termo=1):
    if k == n:
        return 0
    
    resultado = termo + calcular_ex_taylor_otimizado(x, n, k+1, termo * x / (k+1))
...
def criar_tabela_testes(valores_x, valores_n):
    ...
    for x in valores_x:
        for n in valores_n:
            if x >= 0:
                resultado = calcular_ex_taylor_otimizado(x, n)
                esperado = math.exp(x)
